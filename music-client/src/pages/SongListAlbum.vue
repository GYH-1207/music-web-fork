  ...
  <div class="song-list-album">
    ...
    <div class="album-content">
      ...
      <div class="album-score">
        ...
        <div>
          <h3>评价：</h3>
          <div @click="setRank" v-if="this.rank == 0">
...
export default {
  ...
  mounted() {
    getUserRank(this.userId, this.songListId)
      .then((res) => {
        if (res) {
          this.rank = res / 2;
  ...
  methods: {
    ...
    getSongId() {
      listSongDetail(this.songListId)
        .then((res) => {
          // 获取所有歌曲信息的 Promise 数组
          const promises = res.map((item) => this.getSongList(item.songId));
          
          // 等待所有歌曲信息都获取到后再进行排序和提交
          Promise.all(promises)
            .then(() => {
              // 排序函数，按 songPlayCount 降序排序
              this.songLists.sort((a, b) => b.songPlayCount - a.songPlayCount);
    ...
    //根据歌曲id获取歌曲信息
    getSongList(id) {
      return songOfSongId(this.userId, id)
        .then((res) => {
          if (res) {
