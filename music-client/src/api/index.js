...
//============歌曲相关================
//根据歌手id查询歌曲
export const songOfSingerId = (userId, id) => get(`song/singer/detailClient?userId=${userId}&singerId=${id}`);
//根据歌曲id查询歌曲对象
export const songOfSongId = (userId, id) => get(`song/detailClient?userId=${userId}&songId=${id}`);
//根据歌曲名字模糊查询歌曲
...
// ===========================歌曲播放量相关==========================
//新增
export const addSongPlayCount = (params) => post(`/songPlaySum/add`, params);
// 新增用户当前歌曲的播放量
export const setSongPlayCount = (userId, songId) => get(`/songPlaySum/songPlayCountadd?userId=${userId}&songId=${songId}`);
// 查询用户歌曲播放量前三数据
export const selecSongtByUserId = (userId) => get(`/songPlaySum/selectByUserId?userId=${userId}`);
//查询是否已有数据
export const isSongadd = (userId, songId) => get(`/songPlaySum/isadd?userId=${userId}&songId=${songId}`);

// ===========================VIP订单相关==========================
export const createOrder = (params) => post(`/order/add`, params);
export const getNewVipInfo = (userId) => get(`/vip/newVipInfo?userId=${userId}`)
