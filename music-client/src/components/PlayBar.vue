  ...
  <div class="play-bar" :class="{ show: !toggle }">
    ...
    <div class="kongjian">
      ...
      <div class="playing-speed">
        ...
        <!-- <svg class="icon" v-if="playModel==1">
                    <use xlink:href="#953"></use>
                </svg>
                <svg class="icon" v-else-if="playModel==0">
                    <use xlink:href="#1092"></use>
                </svg>
                <svg class="icon" v-else>
                    <use xlink:href="#1232"></use>
                </svg> -->
        <div class="item" @click="togglePlayModel">
          <svg class="icon">
            <use :xlink:href="playModelSVG"></use>
          </svg>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    togglePlayModel() {
      console.log("playModel: " + this.playModel);
      console.log("playModelSVG: " + this.playModelSVG);

      if (this.playModel == 2) {
        this.$store.commit("setPlayModel", 0);
        this.$store.commit("setPlayModelSVG", "#953");
      } else {
        this.$store.commit("setPlayModel", this.playModel + 1);
        this.playModel == 1
          ? this.$store.commit("setPlayModelSVG", "#1092")
          : this.$store.commit("setPlayModelSVG", "#1232");
      }
      console.log("playModel: " + this.playModel);
      console.log("playModelSVG: " + this.playModelSVG);
    },
    //上一首
    prev() {
      if (this.listIndex != -1 && this.listPlayList.length > 1) {
        ...
        } else if (this.playModel == 0) {
          if (this.listIndex > 0) {
            //不是第一首音乐
            this.$store.commit("setListIndex", this.listIndex - 1); //直接返回上一首
          } else {
            //当前是第一首音乐
            this.$store.commit("setListIndex", this.listPlayList.length - 1); //切换到倒数第一首
          }
        } else {
          let index = Math.floor(Math.random() * this.listPlayList.length);
    ...
    next() {
      if (this.listIndex != -1 && this.listPlayList.length > 1) {
        ...
        if (this.playModel == 1) {
          ...
          //列表循环
          if (this.listIndex < this.listPlayList.length - 1) {
            //不是最后一首音乐
      ...
      if (url && url != this.url) {
        ...
        this.$store.commit(
          ...
          this.$store.state.configure.HOST +
            this.listPlayList[this.listIndex].pic
        );
        this.$store.commit(
          "setTitle",
          this.replaceFName(this.listPlayList[this.listIndex].name)
        );
        this.$store.commit(
          "setArtist",
          this.replaceLName(this.listPlayList[this.listIndex].name)
        );
        this.$store.commit(
          "setLyric",
          this.parseLyric(this.listPlayList[this.listIndex].lyric)
        );
        this.$store.commit("setIsPlay", true);
        ...
        if (this.loginIn) {
          getCollectOfUserId(this.userId).then((res) => {
            for (let item of res) {
              if (item.songId == this.listPlayList[this.listIndex].id) {
    ...
    parseLyric(text) {
      ...
      for (let item of lines) {
        ...
        for (let item1 of time) {
          let t = item1.slice(1, -1).split(":"); //取出时间，换算成数组[00:51.641] -> [00,51.641]
          if (value != "") {
            result.push([parseInt(t[0], 10) * 60 + parseFloat(t[1]), value]); //t[0]解析成10进制*60转换成秒把给放入t[1]
          }
        }
      }
      //按照第一个元素--时间--排序
      ...
      if (this.loginIn) {
        ...
        setCollect(params).then((res) => {
          if (res.code == 1) {
            ...
            delCollect(this.userId, this.id).then((res1) => {
              if (res1) {
                console.log(res1);
                this.notify("取消收藏成功", "warning");
                this.$store.commit("setIsActive", false);
              } else {
                this.notify("取消收藏失败", "warning");
              }
            });
          } else {
